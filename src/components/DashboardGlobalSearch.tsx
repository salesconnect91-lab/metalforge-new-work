...
export default function DashboardGlobalSearch() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      (items.data ?? []).forEach((r: any) => next.push({ id:r.id,type:"Item",title:text(r.name)||text(r.sku),subtitle:[r.sku,r.grade,r.size].filter(Boolean).join(" · "),route:"/master-data",icon:Package }));
      (customers.data ?? []).forEach((r: any) => next.push({ id:r.id,type:"Customer",title:text(r.name),subtitle:[r.phone,r.email].filter(Boolean).join(" · "),route:"/master-data/customers",icon:Users }));
      (suppliers.data ?? []).forEach((r: any) => next.push({ id:r.id,type:"Supplier",title:text(r.name),subtitle:[r.phone,r.email].filter(Boolean).join(" · "),route:"/master-data/suppliers",icon:Truck }));
      ...
      setResults(next.slice(0,20)); setLoading(false); setOpen(true);
    },250);
    return () => { cancelled=true; window.clearTimeout(timer); };
  },[q]);

  const openResult=(result:SearchResult)=>{setOpen(false);setQuery("");navigate(result.route)};
  return <div ref={boxRef} className="relative mx-auto w-full max-w-4xl px-4 pt-4 lg:px-5">
    <div className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400"/><input value={query} onChange={(e)=>{setQuery(e.target.value);setOpen(true)}} onFocus={()=>q.length>=2&&setOpen(true)} placeholder="Search anything — invoice, customer, supplier, item, journal, work order..." className="h-12 w-full rounded-xl border border-slate-300 bg-white pl-12 pr-12 text-[14px] font-semibold text-slate-900 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100"/>{query&&<button type="button" onClick={()=>{setQuery("");setResults([])}} className="absolute right-3 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100"><X className="h-4 w-4"/></button>}</div>
    {open&&q.length>=2&&<div className="absolute left-4 right-4 top-[68px] z-[80] max-h-[480px] overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-2xl lg:left-5 lg:right-5">{loading?<div className="px-4 py-8 text-center text-sm font-semibold text-slate-500">Searching MetalForge…</div>:results.length?<><div className="px-3 py-2 text-[11px] font-black uppercase tracking-wider text-slate-400">Search results · {results.length}</div>{results.map(r=>{const Icon=r.icon;return <button key={`${r.type}-${r.id}`} type="button" onClick={()=>openResult(r)} className="group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-blue-50"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600 group-hover:bg-blue-100 group-hover:text-blue-700"><Icon className="h-4 w-4"/></span><span className="min-w-0 flex-1"><span className="block truncate text-[13px] font-black text-slate-900">{r.title}</span><span className="block truncate text-[11px] font-semibold text-slate-500">{r.type}{r.subtitle?` · ${r.subtitle}`:""}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-slate-300 group-hover:text-blue-600"/></button>})}</>:<div className="px-4 py-8 text-center"><div className="text-sm font-black text-slate-700">No result found</div><div className="mt-1 text-xs text-slate-500">Try document number, party name, SKU, journal number or work order.</div></div>}</div>}
  </div>;
}
