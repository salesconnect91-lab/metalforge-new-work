...
function ModuleOnly({ module, children }: { module: ModuleKey; children: ReactNode }) {
  ...
  const unitAllowed = isPlatformOwner || !activeBusinessUnit || activeBusinessUnit.enabled_modules.includes(module);
...
function GlobalExperience() { return <><ErpExperienceBridge /><PrintPreviewController /><LockedPurchaseTaxInputs /></>; }
function DashboardHome() { return <><DashboardGlobalSearch /><Dashboard /></>; }
...
export default function App() {
  ...
  return <><GlobalExperience /><Routes><Route path="/login" element={<Login />} /><Route path="/*" element={<ProtectedRoute><><CompanySwitcher /><BusinessUnitSwitcher /><Layout key={workspaceKey}><Routes>
