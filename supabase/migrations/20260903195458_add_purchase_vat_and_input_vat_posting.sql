-- Purchase VAT parity with Sales Invoice.
-- Existing purchase documents remain non-tax; new Tax Invoices store per-line VAT.
alter table public.purchase_orders
  add column if not exists invoice_type text not null default 'Purchase Invoice',
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname='purchase_orders_invoice_type_check') then
    alter table public.purchase_orders
      add constraint purchase_orders_invoice_type_check
      check (invoice_type in ('Purchase Invoice', 'Tax Invoice'));
  end if;
  if not exists (select 1 from pg_constraint where conname='purchase_orders_tax_percent_check') then
    alter table public.purchase_orders
      add constraint purchase_orders_tax_percent_check
      check (tax_percent between 0 and 100);
  end if;
  if not exists (select 1 from pg_constraint where conname='purchase_order_lines_tax_percent_check') then
    alter table public.purchase_order_lines
      add constraint purchase_order_lines_tax_percent_check
      check (tax_percent between 0 and 100);
  end if;
end $$;
...
BEGIN
  ...
  SELECT
    ...
    ROUND(
      COALESCE(
        SUM(
          CASE WHEN COALESCE(v_order.invoice_type, 'Purchase Invoice') = 'Tax Invoice'
            THEN ROUND(COALESCE(pol.qty, 0) * COALESCE(pol.unit_cost, 0) * COALESCE(pol.tax_percent, 0) / 100, 2)
            ELSE 0
          END
        ),
        0
  ...
  IF EXISTS (
    ...
    WHERE pol.order_id = p_order_id
      ...
      AND (
        ...
        OR COALESCE(pol.tax_percent, 0) < 0
        OR COALESCE(pol.tax_percent, 0) > 100
      )
  ) THEN
    RAISE EXCEPTION
  ...
  IF COALESCE(v_order.transport_charge, 0) < 0
     OR COALESCE(v_order.loading_charge, 0) < 0
     OR COALESCE(v_order.unloading_charge, 0) < 0
     OR COALESCE(v_order.cutting_charge, 0) < 0
     OR COALESCE(v_order.labour_charge, 0) < 0
     OR COALESCE(v_order.handling_charge, 0) < 0
     OR COALESCE(v_order.other_charge, 0) < 0 THEN
    RAISE EXCEPTION 'Purchase charges cannot be negative.';
  END IF;

  v_charge_tax := CASE
    WHEN COALESCE(v_order.invoice_type, 'Purchase Invoice') = 'Tax Invoice'
      THEN ROUND((v_transport_charge + v_general_charges) * COALESCE(v_order.tax_percent, 0) / 100, 2)
    ELSE 0
  END;

  v_tax_total := ROUND(v_item_tax + v_charge_tax, 2);

  IF v_tax_total > 0 AND v_input_vat_account_id IS NULL THEN
    RAISE EXCEPTION 'Input VAT account mapping is missing.';
  END IF;
  ...
  /*
   * Recoverable purchase VAT debit.
   */
  IF v_tax_total > 0 THEN
    INSERT INTO public.journal_lines (
      user_id, entry_id, account, account_id, debit, credit
    )
    SELECT
      v_user_id, v_journal_entry_id, coa.name,
      v_input_vat_account_id, v_tax_total, 0
    FROM public.chart_of_accounts coa
    WHERE coa.id = v_input_vat_account_id
      AND coa.user_id = v_user_id;

    IF NOT FOUND THEN
      RAISE EXCEPTION 'Input VAT account is invalid.';
    END IF;
  END IF;
