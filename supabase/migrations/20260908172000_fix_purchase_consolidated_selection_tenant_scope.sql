...
create function public.get_available_consolidated_purchase_invoices(p_supplier_id uuid default null, p_order_id uuid default null)
returns table(id uuid, invoice_no text, invoice_date date, reference_name text, reference_no text, reference_notes text, subtotal numeric, item_tax numeric, charges_total numeric, charge_tax numeric, total numeric, linked_purchase_order_id uuid, supplier_id uuid, invoice_type text)
language plpgsql
...
begin
  ...
  return query
  select h.id,h.invoice_no,h.invoice_date,h.reference_name,h.reference_no,h.reference_notes,h.subtotal,h.item_tax,h.charges_total,h.charge_tax,h.total,l.purchase_order_id,h.supplier_id,h.invoice_type
  ...
  where h.company_id=co and h.business_unit_id=bu
