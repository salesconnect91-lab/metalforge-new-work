  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const { data: order, error: orderError } = await supabase.from("purchase_orders").insert({
        order_no: orderNo, supplier_id: supplierId, order_date: orderDate, status: "draft", invoice_type: invoiceType,
        tax_percent: invoiceType === "Tax Invoice" ? Number(globalTaxPercent) || 0 : 0,
        total: grandTotal, ...buildChargePayload(charges),
      }).select().single();
    ...
    } catch (e: any) { setError(e?.message || "Failed to save Main Purchase Invoice."); }
    finally { setSaving(false); }
  };

  return <div>
    <Link to="/purchase" className="mb-4 inline-block text-sm text-primary-600">← Back to Purchase</Link>
    <PageHeader title="Main Purchase Invoice / مین خریداری انوائس" subtitle="Direct purchase items + separately posted Consolidated Purchase Invoices" />
