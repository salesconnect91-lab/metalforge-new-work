...
export default function WarehouseStock(){
 ...
 return <div className="space-y-5">
  ...
  <div className="rounded-2xl border bg-white p-4"><div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="flex items-center gap-2 rounded-xl border bg-slate-50 px-3 py-2.5"><Search className="h-4 w-4 text-slate-400"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search item, SKU..." className="w-full bg-transparent text-sm outline-none"/></div><select value={warehouseFilter} onChange={e=>{setWarehouseFilter(e.target.value);setGodownFilter("all")}} className="rounded-xl border px-3 py-2.5"><option value="all">All Warehouses</option>{warehouses.map(w=><option key={w.id} value={w.id}>{bilingual(w.name,w.name_urdu)}</option>)}</select><select value={godownFilter} onChange={e=>setGodownFilter(e.target.value)} className="rounded-xl border px-3 py-2.5"><option value="all">All Godowns</option>{filteredGodownsForFilter.map(g=><option key={g.id} value={g.id}>{bilingual(g.name,g.name_urdu)}</option>)}</select><select value={itemFilter} onChange={e=>setItemFilter(e.target.value)} className="rounded-xl border px-3 py-2.5"><option value="all">All Items</option>{items.map(i=><option key={i.id} value={i.id}>{i.sku} — {bilingual(i.name,i.name_urdu)} — {clean(i.unit)||"—"}</option>)}</select></div><div className="mt-3 flex justify-end gap-2"><button type="button" onClick={()=>void fetchData()} className="btn-secondary"><RefreshCw className="h-4 w-4"/>Refresh</button><button type="button" onClick={exportExcel} className="btn-secondary"><FileSpreadsheet className="h-4 w-4"/>Export Excel</button><button type="button" onClick={printPdf} className="btn-secondary"><Printer className="h-4 w-4"/>Print / PDF</button></div></div>
  ...
  {modalOpen&&<div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/60 p-4"><div className="max-h-[94vh] w-full max-w-xl overflow-y-auto rounded-2xl border bg-white"><div className="flex items-center justify-between border-b px-5 py-4"><div><h2 className="font-bold">Controlled Stock Adjustment / اسٹاک ایڈجسٹمنٹ</h2><p className="mt-1 text-xs text-slate-500">Enter only the quantity to Add or Remove. Source-document stock cannot be created here.</p></div><button type="button" onClick={closeModal}><X className="h-5 w-5"/></button></div><form onSubmit={saveAdjustment} className="space-y-4 p-5">
   <div><label className="mb-1.5 block text-sm font-bold">Item / آئٹم *</label><select required value={form.item_id} onChange={e=>setForm(c=>({...c,item_id:e.target.value}))} className="w-full rounded-xl border px-3 py-2.5"><option value="">Select item</option>{items.map(i=><option key={i.id} value={i.id}>{i.sku} — {bilingual(i.name,i.name_urdu)} — {clean(i.unit)||"—"}</option>)}</select></div>
   <div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1.5 block text-sm font-bold">Warehouse / ویئرہاؤس *</label><select required value={form.warehouse_id} onChange={e=>{const wid=e.target.value;const first=godowns.find(g=>g.warehouse_id===wid);setForm(c=>({...c,warehouse_id:wid,godown_id:first?.id??""}))}} className="w-full rounded-xl border px-3 py-2.5"><option value="">Select warehouse</option>{warehouses.map(w=><option key={w.id} value={w.id}>{bilingual(w.name,w.name_urdu)}</option>)}</select></div><div><label className="mb-1.5 block text-sm font-bold">Godown / گودام *</label><select required value={form.godown_id} disabled={!form.warehouse_id} onChange={e=>setForm(c=>({...c,godown_id:e.target.value}))} className="w-full rounded-xl border px-3 py-2.5"><option value="">Select godown</option>{modalGodowns.map(g=><option key={g.id} value={g.id}>{bilingual(g.name,g.name_urdu)}</option>)}</select></div></div>
   <div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1.5 block text-sm font-bold">Adjustment Type / قسم *</label><select value={form.action} onChange={e=>setForm(c=>({...c,action:e.target.value as AdjustmentAction}))} className="w-full rounded-xl border px-3 py-2.5"><option value="remove">Stock Remove / اسٹاک کم کریں</option><option value="add">Stock Add / اسٹاک بڑھائیں</option></select></div><div><label className="mb-1.5 block text-sm font-bold">Quantity / مقدار *</label><div className="flex overflow-hidden rounded-xl border"><input required type="number" min="0.001" step="0.001" value={form.qty} onChange={e=>setForm(c=>({...c,qty:e.target.value}))} className="min-w-0 flex-1 border-0 px-3 py-2.5 outline-none"/><span className="flex min-w-[64px] items-center justify-center border-l bg-slate-50 px-3 font-bold">{uom}</span></div></div></div>
