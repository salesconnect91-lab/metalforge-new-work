...
export default function InvoiceFinancialSummary({
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white">
        ...
        {customerId && totals.outstanding > 0 && (
          <div className="flex flex-wrap gap-1.5">
            <Link
              ...
              className="inline-flex items-center justify-center gap-1.5 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[12px] font-bold text-emerald-700 hover:bg-emerald-100"
            >
              <Banknote className="h-3.5 w-3.5" />
              Receive Payment — رقم وصول کریں
            </Link>
            <button type="button" onClick={()=>void createReminder()} disabled={sendingReminder} className="inline-flex items-center justify-center gap-1.5 rounded-md border border-amber-200 bg-amber-50 px-3 py-1.5 text-[12px] font-bold text-amber-700 hover:bg-amber-100 disabled:opacity-50">
      ...
      {error && (
        <div className="border-b border-amber-100 bg-amber-50 px-3 py-2 text-[12px] text-amber-700">
      ...
      <div className="grid grid-cols-2 gap-2 p-3 md:grid-cols-4 xl:grid-cols-7">
        {[
          ...
          <div key={String(label)} className="rounded-md border border-slate-100 bg-slate-50 p-2.5">
            ...
            <div className="text-[12px] text-slate-400">{urdu}</div>
      ...
      <div className="grid grid-cols-1 gap-3 border-t border-slate-100 p-3 lg:grid-cols-[1fr_1.6fr]">
        ...
        <div className="rounded-md border border-slate-100 overflow-hidden">
          ...
          {payments.length === 0 ? (
            <div className="p-4 text-center text-[12px] text-slate-400">No invoice payments recorded. / کوئی انوائس ادائیگی ریکارڈ نہیں۔</div>
          ) : (
      ...
      {financial?.overdue_days && n(financial.overdue_days) > 0 && (
        <div className="border-t border-rose-100 bg-rose-50 px-3 py-2 text-[12px] font-semibold text-rose-700">
