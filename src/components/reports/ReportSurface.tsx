...
function isActionHeader(label: string) { return /^(actions?|action|عمل|کارروائی)$/i.test(label.trim()); }
...
export default function ReportSurface({ children }: ReportSurfaceProps) {
  ...
  const fullRowsForExport = () => {
    const table = reportContent()?.querySelector("table"); if (!table) return [] as string[][];
    ...
    return Array.from(table.querySelectorAll("tr")).filter(row => showTotals || !(row.matches("[data-report-total],.report-total-row") || row.closest("tfoot"))).map(row => Array.from(row.children).filter((_cell, index) => !actionIndexes.has(index)).map(cell => (cell.textContent || "").replace(/\s+/g, " ").trim())).filter(row => row.some(value => value !== ""));
  };
