...
where p.company_id=public.current_company_id()
  and p.business_unit_id=public.current_business_unit_id()
union all
select a.company_id,
       a.business_unit_id,
       a.employee_id,
       ...
       e.department,
       a.salary_month,
       a.salary_month as entry_date,
       a.voucher_no as document_no,
       'Accrual'::text as entry_type,
       a.monthly_salary as debit,
       0::numeric as credit,
       a.journal_entry_id
from public.employee_salary_accruals a
join public.employees e on e.id=a.employee_id
where a.company_id=public.current_company_id()
  and a.business_unit_id=public.current_business_unit_id();
