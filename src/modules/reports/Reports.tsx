...
export default function Reports() {
  ...
  return <div className="space-y-4 pb-12">
    ...
    <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
      <button onClick={()=>setTab("sales")} className={`rounded-lg border p-3 text-left ${tab==="sales"?"border-blue-300 bg-blue-50":"border-slate-200 bg-white"}`}><TrendingUp className="h-4 w-4 text-blue-600"/><div className="mt-2 text-xs font-bold">Sales & Margin / فروخت اور مارجن</div><div className="text-[9px] text-slate-400">فروخت و منافع</div></button>
      <button onClick={()=>setTab("stock")} className={`rounded-lg border p-3 text-left ${tab==="stock"?"border-blue-300 bg-blue-50":"border-slate-200 bg-white"}`}><Boxes className="h-4 w-4 text-emerald-600"/><div className="mt-2 text-xs font-bold">Stock / Godown / اسٹاک اور گودام</div><div className="text-[9px] text-slate-400">اسٹاک / گودام</div></button>
      <button onClick={()=>setTab("receivables")} className={`rounded-lg border p-3 text-left ${tab==="receivables"?"border-blue-300 bg-blue-50":"border-slate-200 bg-white"}`}><Users className="h-4 w-4 text-amber-600"/><div className="mt-2 text-xs font-bold">Receivables Aging / وصولیوں کی مدت</div><div className="text-[9px] text-slate-400">وصولی ایجنگ</div></button>
      <button onClick={()=>setTab("charges")} className={`rounded-lg border p-3 text-left ${tab==="charges"?"border-blue-300 bg-blue-50":"border-slate-200 bg-white"}`}><BarChart3 className="h-4 w-4 text-violet-600"/><div className="mt-2 text-xs font-bold">Service Charges / سروس چارجز</div><div className="text-[9px] text-slate-400">لوڈنگ / کٹنگ وغیرہ</div></button>
    </div>
    {loading ? <div className="rounded-lg border bg-white p-8 text-center text-xs text-slate-400">Loading reports / رپورٹس لوڈ ہو رہی ہیں…</div> : tab === "sales" ? <div className="rounded-lg border bg-white overflow-hidden"><div className="grid grid-cols-3 gap-2 border-b p-3"><div><span className="text-[9px] text-slate-400">Sales / فروخت</span><div className="font-bold">{formatCurrency(salesSummary.sales)}</div></div><div><span className="text-[9px] text-slate-400">Cost / لاگت</span><div className="font-bold">{formatCurrency(salesSummary.cost)}</div></div><div><span className="text-[9px] text-slate-400">Gross Profit / مجموعی منافع</span><div className="font-bold text-emerald-700">{formatCurrency(salesSummary.profit)}</div></div></div><div className="overflow-x-auto"><table className="w-full text-[10px]"><thead className="bg-slate-50"><tr><th className="p-2 text-left">Invoice / انوائس</th><th className="p-2 text-left">Customer / گاہک</th><th className="p-2 text-left">Salesperson / سیلز پرسن</th><th className="p-2 text-right">Sales / فروخت</th><th className="p-2 text-right">Cost / لاگت</th><th className="p-2 text-right">Profit / نفع</th><th className="p-2 text-right">Margin %</th></tr></thead><tbody>{sales.map(r=><tr key={r.sales_order_id} className="border-t"><td className="p-2 font-semibold">{r.invoice_no}</td><td className="p-2">{r.customer_name||"—"}</td><td className="p-2">{r.sales_person||"—"}</td><td className="p-2 text-right">{formatCurrency(n(r.sales_amount))}</td><td className="p-2 text-right">{formatCurrency(n(r.cost_amount))}</td><td className="p-2 text-right text-emerald-700">{formatCurrency(n(r.gross_profit))}</td><td className="p-2 text-right">{n(r.margin_percent).toFixed(2)}%</td></tr>)}</tbody></table></div></div> : tab === "stock" ? <div className="rounded-lg border bg-white overflow-hidden"><div className="grid grid-cols-2 gap-2 border-b p-3"><div><span className="text-[9px] text-slate-400">Quantity / مقدار</span><div className="font-bold">{stockSummary.qty.toLocaleString()}</div></div><div><span className="text-[9px] text-slate-400">Stock Value / اسٹاک مالیت</span><div className="font-bold">{formatCurrency(stockSummary.value)}</div></div></div><div className="overflow-x-auto"><table className="w-full text-[10px]"><thead className="bg-slate-50"><tr><th className="p-2 text-left">Item / آئٹم</th><th className="p-2 text-left">Grade / گریڈ</th><th className="p-2 text-left">Godown / گودام</th><th className="p-2 text-right">Qty / مقدار</th><th className="p-2 text-right">Value / مالیت</th></tr></thead><tbody>{stock.map((r,i)=><tr key={`${r.item_id}-${r.godown}-${i}`} className="border-t"><td className="p-2">{r.item_name}</td><td className="p-2">{r.grade||"—"}</td><td className="p-2 font-semibold">{r.godown}</td><td className="p-2 text-right">{n(r.quantity).toLocaleString()}</td><td className="p-2 text-right">{formatCurrency(n(r.stock_value))}</td></tr>)}</tbody></table></div></div> : tab === "receivables" ? <div className="rounded-lg border bg-white overflow-hidden"><div className="grid grid-cols-2 gap-2 border-b p-3"><div><span className="text-[9px] text-slate-400">Outstanding / بقایا</span><div className="font-bold text-rose-700">{formatCurrency(receivableSummary.out)}</div></div><div><span className="text-[9px] text-slate-400">Today's Received / آج کی وصولی</span><div className="font-bold text-emerald-700">{formatCurrency(receivableSummary.today)}</div></div></div><div className="overflow-x-auto"><table className="w-full text-[10px]"><thead className="bg-slate-50"><tr><th className="p-2 text-left">Invoice / انوائس</th><th className="p-2 text-left">Customer / گاہک</th><th className="p-2 text-right">Invoice / انوائس</th><th className="p-2 text-right">Paid / ادا شدہ</th><th className="p-2 text-right">Outstanding / بقایا</th><th className="p-2 text-right">Overdue Days / زائد المیعاد دن</th></tr></thead><tbody>{receivables.map(r=><tr key={r.sales_order_id} className="border-t"><td className="p-2 font-semibold">{r.invoice_no}</td><td className="p-2">{r.customer_name}</td><td className="p-2 text-right">{formatCurrency(n(r.invoice_amount))}</td><td className="p-2 text-right text-emerald-700">{formatCurrency(n(r.paid_amount))}</td><td className="p-2 text-right text-rose-700">{formatCurrency(n(r.outstanding_amount))}</td><td className="p-2 text-right">{n(r.overdue_days)}</td></tr>)}</tbody></table></div></div> : <div className="rounded-lg border bg-white overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-[10px]"><thead className="bg-slate-50"><tr><th className="p-2 text-left">Party / پارٹی</th><th className="p-2 text-left">Type / قسم</th><th className="p-2 text-right">Charges / چارجز</th></tr></thead><tbody>{charges.map(r=><tr key={r.service_party_id} className="border-t"><td className="p-2 font-semibold">{r.name}</td><td className="p-2 capitalize">{r.party_type}</td><td className="p-2 text-right">{formatCurrency(n(r.charges_total))}</td></tr>)}</tbody></table></div></div>}
  </div>;
}
