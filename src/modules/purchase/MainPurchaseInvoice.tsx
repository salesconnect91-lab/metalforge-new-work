      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="card p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <div><label className="label">Supplier / سپلائر</label><SearchableSelect className="input" required value={supplierId} onChange={(e) => setSupplierId(e.target.value)}><option value="">— Select supplier —</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</SearchableSelect></div>
            ...
            <div><label className="label">Invoice Type / قسم</label><SearchableSelect className="input" value={invoiceType} onChange={(e) => setInvoiceType(e.target.value as "Purchase Invoice" | "Tax Invoice")}><option value="Purchase Invoice">Without Tax / بغیر ٹیکس</option><option value="Tax Invoice">With Tax / ٹیکس کے ساتھ</option></SearchableSelect></div>
          </div>
        ...
        <div className="card p-6">
          ...
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b"><th className="py-2 text-left">Item</th><th>Godown</th><th className="text-right">Qty</th><th className="text-right">Unit Cost</th>{invoiceType === "Tax Invoice" && <th className="text-right">VAT</th>}<th className="text-right">Amount</th><th /></tr></thead><tbody>{rows.map((row, index) => {
            ...
            return <tr key={index} className="border-b border-slate-100"><td className="py-2 pr-2"><SearchableSelect className="input" value={row.item_id} onChange={(e) => updateLine(index, "item_id", e.target.value)}><option value="">— Select —</option>{items.map((item) => <option key={item.id} value={item.id}>{item.name}{item.sku ? ` (${item.sku})` : ""}</option>)}</SearchableSelect></td><td className="px-2"><SearchableSelect className="input" value={row.godown_id} onChange={(e) => updateLine(index, "godown_id", e.target.value)}><option value="">— Select —</option>{godowns.map((godown) => <option key={godown.id} value={godown.id}>{godown.name}</option>)}</SearchableSelect></td><td className="px-2"><input className="input w-24 text-right" type="number" min="0" step="0.01" value={row.qty} onChange={(e) => updateLine(index, "qty", e.target.value)} /></td><td className="px-2"><input className="input w-28 text-right" type="number" min="0" step="0.01" value={row.unit_cost} onChange={(e) => updateLine(index, "unit_cost", e.target.value)} /></td>{invoiceType === "Tax Invoice" && <td className="px-2"><input className="input w-20 bg-slate-50 text-right cursor-not-allowed" disabled value={row.tax_percent} /></td>}<td className="px-2 text-right font-medium">{formatCurrency(base + tax)}</td><td className="pl-2 text-right">{rows.length > 1 && <button type="button" className="text-error-600" onClick={() => setRows(rows.filter((_, i) => i !== index))}>Remove</button>}</td></tr>;
          })}</tbody></table></div>
        </div>
