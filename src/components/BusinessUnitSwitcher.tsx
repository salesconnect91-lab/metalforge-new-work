...
export default function BusinessUnitSwitcher(){
  ...
  return <div className="fixed z-40 hidden min-w-[260px] md:block" style={position?{left:position.x,top:position.y,right:"auto"}:{right:390,top:7}} data-no-bilingual data-no-print data-no-print-overlay>
    <div className="relative rounded-xl border border-blue-200/90 bg-blue-50/95 px-2.5 py-1.5 shadow-sm backdrop-blur-xl">
      <div className="mb-0.5 flex cursor-move touch-none select-none items-center gap-1.5 px-1 text-[12px] font-bold uppercase tracking-[0.13em] text-blue-500" title="Drag to move" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerCancel={stopDrag}><BriefcaseBusiness size={10}/><span>{canSwitch?"Business Unit / بزنس یونٹ":"Business Workspace / بزنس ورک اسپیس"}</span><Move size={11} className="ml-auto shrink-0 opacity-70"/></div>
      {canSwitch?<div className="relative"><SearchableSelect aria-label="Active business unit" className="h-7 w-full appearance-none rounded-md border-0 bg-transparent pl-1 pr-8 text-[12px] font-black text-blue-950 outline-none focus:ring-0" value={activeBusinessUnit.business_unit_id} disabled={switchingBusinessUnit} onChange={e=>{const id=e.target.value;if(!id)return;setError("");void switchBusinessUnit(id).then(({error:x})=>{if(x){setError(x);return}navigate("/")})}}>{availableBusinessUnits.map(u=><option key={u.business_unit_id} value={u.business_unit_id}>{u.business_unit_name} ({u.business_unit_code})</option>)}</SearchableSelect><span className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 text-blue-500">{switchingBusinessUnit?<Loader2 size={13} className="animate-spin"/>:<ChevronDown size={13}/>}</span></div>:<div className="h-7 truncate px-1 text-[12px] font-black leading-7 text-blue-950">{activeBusinessUnit.business_unit_name} ({activeBusinessUnit.business_unit_code})</div>}
