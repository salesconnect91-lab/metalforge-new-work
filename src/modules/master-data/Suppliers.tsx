...
export default function Suppliers() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editing) {
      ...
      const { error } = await supabase.rpc("create_supplier_with_ap", {
        p_name: payload.name,
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of valid) {
        const { error: rpcError } = await supabase.rpc("create_supplier_with_ap", {
          p_name: row.name,
          p_email: row.email,
          p_phone: row.phone,
      ...
      await fetchRows();
      alert(
        `${valid.length} suppliers imported successfully / کامیابی سے امپورٹ ہوگئے۔`
      );
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Vendor accounts / سپلائر اکاؤنٹس"
        action={
      ...
      <ConfirmModal
        open={!!deleteId}
        title={`${rows.find((row) => row.id === deleteId)?.is_active === false ? "Activate" : "Deactivate"} Supplier`}
        message="Historical transactions will remain safe. Inactive suppliers cannot be selected for new transactions."
