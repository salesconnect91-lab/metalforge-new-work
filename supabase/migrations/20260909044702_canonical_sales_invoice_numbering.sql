...
begin
  ...
  -- Tax invoices keep a TAX sequence. All non-tax sales invoices share the
  -- canonical INV sequence; the former Cash Bill input is compatibility only.
  ...
  if new.order_no is null or btrim(new.order_no) = '' or new.order_no like '%-AUTO' then
    perform pg_advisory_xact_lock(hashtextextended(v_company_id::text||':'||v_unit::text||':sales-order:'||v_prefix,0));
