  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const [items, customers, suppliers, sales, purchases, journals, workOrders, cuttingOrders, gatePasses, returns, movements] = await Promise.all([
        ...
        supabase.from("sales_orders").select("id,order_no,status,total,fbr_invoice_no").or(`order_no.ilike.${like},status.ilike.${like},fbr_invoice_no.ilike.${like}`).limit(6),
        supabase.from("purchase_orders").select("id,order_no,status,total,supplier_invoice_no").or(`order_no.ilike.${like},status.ilike.${like},supplier_invoice_no.ilike.${like}`).limit(6),
