...
function SearchPick({value,setValue,options,placeholder}:{value:string;setValue:(v:string)=>void;options:string[];placeholder:string}){const id=`dl-${placeholder.replace(/\W/g,"-")}`;return <><input list={id} className="input w-full" value={value} onChange={e=>setValue(e.target.value)} placeholder={placeholder}/><datalist id={id}>{[...new Set(options)].map(o=><option key={o} value={o}/>)}</datalist></>}
