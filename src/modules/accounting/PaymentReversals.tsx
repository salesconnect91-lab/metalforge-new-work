...
export default function PaymentReversals(){
  ...
  return <div className="space-y-4"><PageHeader title="Payment Voucher Reversals / ادائیگی واپسی" subtitle="Reverse a posted customer receipt or supplier payment through a separate audited journal. Original vouchers are never edited or deleted."/>
    {error&&<ErrorBanner message={error}/>}<div className="card flex flex-wrap items-center gap-2 p-4 print:hidden"><input className="input min-w-64 flex-1" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search voucher, party, method..."/><SearchableSelect className="input w-auto" value={kind} onChange={e=>setKind(e.target.value as any)}><option value="all">All payments</option><option value="Customer Receipt">Customer Receipts</option><option value="Supplier Payment">Supplier Payments</option></SearchableSelect><button className="btn btn-secondary" onClick={()=>void load()}><RefreshCw size={15}/>Refresh</button></div>
