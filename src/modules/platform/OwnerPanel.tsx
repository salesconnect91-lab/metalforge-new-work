...
export default function OwnerPanel() {
  ...
  const createUser = async () => {
    if (!selectedCompanyId || !user.email.trim() || user.password.length < 8) { setError("Select company, enter email and minimum 8 character password."); return; }
    setSaving(true); setError("");
  ...
  return <div className="space-y-6">
    <PageHeader title="Owner Control Center" subtitle="Platform identity, companies, business workspaces, subscriptions, users, accounting governance, opening data and lifecycle"/>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {companies.map(target => {
          ...
          return <div key={target.id} className="rounded-xl border p-4">
            <div className="flex flex-wrap justify-between gap-3"><div><div className="font-semibold">{target.name} <span className="text-xs text-slate-400">({target.code})</span></div><div className="text-xs text-slate-500">{target.status} · max {target.max_users} users</div></div><div className="flex flex-wrap gap-2"><button className="btn-secondary" disabled={saving} onClick={() => void patchCompanyStatus(target, target.status === "suspended" ? "active" : "suspended")}>{target.status === "suspended" ? "Activate" : "Suspend"}</button><CompanyDeleteControl companyId={target.id} companyName={target.name} companyCode={target.code} onDeleted={async () => { await load(); await refreshAccess(); }}/></div></div>
            <div className="mt-4 space-y-2">{targetMemberships.map(membership => { const profile = profileMap.get(membership.user_id); return <div key={membership.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-slate-50 p-2"><div className="text-sm">{profile?.full_name || profile?.email || membership.user_id}</div><div className="flex gap-2"><SearchableSelect className="input h-8 py-1 text-xs" value={membership.role} onChange={event => void updateMember(membership, { role: event.target.value })}>{roles.map(role => <option key={role}>{role}</option>)}</SearchableSelect><button className="btn-secondary h-8" onClick={() => void updateMember(membership, { is_active: !membership.is_active })}>{membership.is_active ? "Disable" : "Enable"}</button></div></div>; })}{targetMemberships.length === 0 && <div className="text-xs text-slate-400">No company users assigned yet.</div>}</div>
          </div>;
        })}
      </div>
    </section>

    <section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Selected company controls</h2><SearchableSelect className="input mt-3 w-full sm:w-96" value={selectedCompanyId} onChange={event => setSelectedCompanyId(event.target.value)}>{companies.map(target => <option key={target.id} value={target.id}>{target.name} ({target.code})</option>)}</SearchableSelect></section>
    ...
    <section className="rounded-xl border bg-white p-4 shadow-sm"><h2 className="font-semibold">Create company/group login ID</h2><p className="mt-1 text-xs text-slate-500">Use this only for company/group users who may access more than one assigned business. For a Steel-only or Transport-only ID, use Dedicated Business Login IDs above.</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><input className="input" placeholder="Full name" value={user.full_name} onChange={event => setUser({ ...user, full_name: event.target.value })}/><input className="input" type="email" placeholder="Email / login ID" value={user.email} onChange={event => setUser({ ...user, email: event.target.value })}/><input className="input" type="password" placeholder="Temporary password" value={user.password} onChange={event => setUser({ ...user, password: event.target.value })}/><SearchableSelect className="input" value={user.role} onChange={event => setUser({ ...user, role: event.target.value })}>{roles.map(role => <option key={role}>{role}</option>)}</SearchableSelect></div><button className="btn-primary mt-3" disabled={saving || !selectedCompanyId} onClick={() => void createUser()}>Create Group User</button></section>

    <section className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">Opening party balance migration</h2><p className="mt-1 text-xs text-slate-500">Use the dedicated validated migration screen for Excel/CSV preview, duplicate checks and posted opening journal creation.</p></div><Link className="btn-secondary" to="/owner/opening-balances"><FileSpreadsheet className="h-4 w-4"/>Open Migration Tool</Link></div></section>
