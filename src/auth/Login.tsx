...
export default function Login() {
  ...
  const signInWithPassword = async (event: FormEvent) => { event.preventDefault(); if (!normalizedEmail || !password) return; clearStatus(); setLoading(true); const { data, error: signInError } = await supabase.auth.signInWithPassword({ email: normalizedEmail, password }); setLoading(false); if (signInError) { setError(signInError.message); return; } if (data.session) { navigate("/", { replace: true }); return; } setError("Login could not be completed. Please try again."); };
  ...
  return <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4"><div className="w-full max-w-md">
    <div className="mb-6 text-center">
      {showBranding && branding.logo_url && <img src={branding.logo_url} alt={branding.erp_name || "ERP"} className="mx-auto max-h-40 max-w-[13rem] rounded-3xl object-contain shadow-lg" />}
      {showBranding && branding.erp_name && <h1 className={`${branding.logo_url ? "mt-3" : ""} text-2xl font-black text-slate-900`}>{branding.erp_name}</h1>}
      {showBranding && branding.show_tagline && branding.tagline && <p className="mt-1 text-sm font-medium text-slate-500">{branding.tagline}</p>}
      <p className={`${showBranding ? "mt-3" : ""} text-slate-500`}>Secure ERP Login</p>
    </div>
    <div className="card p-6">
      <div className="mb-5 grid grid-cols-2 rounded-lg bg-slate-100 p-1 text-sm font-semibold"><button type="button" onClick={() => changeMode("password")} className={`rounded-md px-3 py-2 ${mode === "password" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>Password</button><button type="button" onClick={() => changeMode("otp")} className={`rounded-md px-3 py-2 ${mode === "otp" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>Email OTP</button></div>
      ...
      {mode === "password" ? <form onSubmit={signInWithPassword} className="space-y-4"><div><label className="label">Login ID / Email</label><input type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="input" placeholder="name@gmail.com" /></div><div><label className="label">Password</label><input type="password" required autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} className="input" placeholder="Enter password" /></div><div className="flex justify-end"><button type="button" disabled={loading} onClick={() => void sendPasswordReset()} className="text-xs font-semibold text-primary-600 hover:text-primary-700 disabled:opacity-50">Forgot password?</button></div><button type="submit" disabled={loading || !normalizedEmail || !password} className="btn-primary w-full">{loading ? "Signing in…" : "Sign In"}</button></form> : !otpSent ? <form onSubmit={sendOtp} className="space-y-4"><div><label className="label">Login ID / Email</label><input type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} className="input" placeholder="name@gmail.com" /></div><button type="submit" disabled={loading || !normalizedEmail} className="btn-primary w-full">{loading ? "Sending OTP…" : "Send Login OTP"}</button></form> : <form onSubmit={verifyOtp} className="space-y-4"><div><label className="label">6-digit OTP</label><input inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} className="input text-center text-xl tracking-[0.35em]" placeholder="123456" autoFocus /></div><button type="submit" disabled={loading || otp.length !== 6} className="btn-primary w-full">{loading ? "Verifying…" : "Verify & Login"}</button><div className="flex items-center justify-between gap-3 text-xs"><button type="button" onClick={changeLoginId} className="font-medium text-slate-500 hover:text-slate-800">Change Login ID</button><button type="button" disabled={loading} onClick={() => void sendOtp()} className="font-semibold text-primary-600 hover:text-primary-700 disabled:opacity-50">Resend OTP</button></div></form>}
    </div>
    <p className="mt-5 text-center text-xs text-slate-400">Access is allowed only for registered {showBranding && branding.erp_name ? `${branding.erp_name} ` : "ERP "}users.</p>
  </div></div>;
}
