...
export default function OrderBook({type}:{type:OrderType}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      const first=settingsRes.error||partyRes.error||itemRes.error||uomRes.error||orderRes.error||salespersonRes.error;
  ...
  const itemUnit=(id:string)=>items.find(i=>i.id===id)?.unit?.trim()||"kg";
  ...
  const createOrder=async()=>{
    ...
    try{
      ...
      const payload=valid.map(l=>({
        order_id:h.id,item_id:l.item_id,item_name:itemName(l.item_id),ordered_qty:n(l.qty),uom:l.uom.trim()||itemUnit(l.item_id),
  ...
  const addSpot=async()=>{
    ...
    setSaving(true); setError(null);
    try{
      const {error}=await supabase.from("order_book_commitments").insert({order_id:spotOrder.id,item_id:spotLine.item_id,item_name:itemName(spotLine.item_id),ordered_qty:n(spotLine.qty),uom:spotLine.uom||itemUnit(spotLine.item_id),rate_status:spotLine.rate_status,agreed_rate:spotLine.rate_status==="agreed"?n(spotLine.rate):null,effective_at:spotLine.rate_status==="agreed"?new Date(`${spotLine.effective_date}T00:00:00`).toISOString():null,source:"spot",remarks:spotLine.remarks.trim()||null});
