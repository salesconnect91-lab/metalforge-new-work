...
function applyInvoiceLayout(clone: HTMLElement) {
  const hasInvoice =
    clone.matches(".print-document") ||
    !!clone.querySelector(".print-document") ||
    clone.id === "consolidated-purchase-print-root" ||
    !!clone.querySelector("#consolidated-purchase-print-root");
  if (!hasInvoice) return;

  Object.assign(clone.style, {
    display: "block", visibility: "visible", width: "100%", maxWidth: "100%", margin: "0", padding: "0",
  });

  if (clone.matches(".print-document")) {
    Object.assign(clone.style, {
      display: "block", visibility: "visible", position: "static", width: "100%", maxWidth: "100%", margin: "0", padding: "0",
    });
  }

  setCss(clone, ".print-document", {
    display: "block", visibility: "visible", position: "static", width: "100%", maxWidth: "100%", margin: "0", padding: "0",
  });
  setCss(clone, ".print-page", {
    display: "block", visibility: "visible", width: "100%", maxWidth: "190mm", margin: "0 auto", padding: "0",
    color: "#111827", background: "#fff", fontFamily: "Arial, Helvetica, sans-serif", fontSize: "8.5pt", lineHeight: "1.3",
  });
  setCss(clone, ".print-header", {
    display: "grid", gridTemplateColumns: "minmax(0,1fr) auto", alignItems: "start", gap: "8mm",
    margin: "0 0 3mm", padding: "0 0 3mm", borderBottom: "1.2px solid #111827",
  });
  ...
  setCss(clone, ".print-meta-row", { display: "grid", gridTemplateColumns: "38mm minmax(0,1fr)", gap: "2mm", padding: ".7mm 0" });
  ...
  setCss(clone, ".print-party-addr,.print-party-phone,.print-party-email", { display: "block", marginTop: ".7mm", fontSize: "7pt", lineHeight: "1.3" });

  setCss(clone, ".invoice-items-wrap", {
    display: "block", width: "100%", margin: "0 0 3mm", borderTop: "1px solid #cbd5e1", borderLeft: "1px solid #cbd5e1",
  });
  ...
  setCss(clone, ".invoice-items-head", { display: "grid", background: "#eef2f7", fontWeight: "800" });
  setCss(clone, ".invoice-items-row", { display: "grid" });
  setCss(clone, ".invoice-items-grid > div", {
    display: "block", minWidth: "0", boxSizing: "border-box", padding: "1.7mm 1.2mm",
    borderRight: "1px solid #cbd5e1", borderBottom: "1px solid #cbd5e1", fontSize: "7pt", lineHeight: "1.2", overflow: "hidden",
  });
  ...
  setCss(clone, ".print-charges-side,.print-totals-side", { display: "block", minWidth: "0" });
  setCss(clone, ".print-charges-box,.print-totals-side", { border: "1px solid #cbd5e1", padding: "2mm" });
  setCss(clone, ".print-charge-row,.print-total-row", { display: "flex", justifyContent: "space-between", gap: "3mm", padding: ".9mm 0", borderBottom: "1px solid #edf0f4", fontSize: "7.2pt" });
...
html,body{background:#fff!important;color:#0f172a!important;margin:0!important;padding:0!important;width:100%!important;font-family:Arial,Helvetica,sans-serif!important}
*,*::before,*::after{box-sizing:border-box!important}
body *{visibility:visible!important}
.mf-print-output{display:block!important;visibility:visible!important;background:#fff!important;width:190mm!important;max-width:190mm!important;margin:0 auto!important;overflow:visible!important}
...
export default function PrintPreviewController() {
  ...
  useEffect(() => {
    ...
    const openPreview = () => {
      ...
      if (!target) return previousPrint.call(window);
  ...
  const printNow = () => {
    ...
    const doPrint = async () => {
      ...
      } catch { /* non-critical print asset */ }
