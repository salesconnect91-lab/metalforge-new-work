...
export default function SupplierAgingReport() {
  ...
  const load = useCallback(async () => {
    ...
    setData((agingResult.data ?? []) as AgingRow[]);
  ...
  const rows = useMemo(() => data.filter(r => {
    ...
    return [r.invoice_no, r.invoice_date, r.supplier_name, r.payment_status, r.aging_status, r.aging_bucket, r.due_date]
      .some(v => String(v ?? "").toLowerCase().includes(search));
  }), [data, q, from, to, supplier, paymentStatus, agingBucket]);

  const summary = useMemo(() => rows.reduce((a, r) => {
    a.invoiced += num(r.invoice_amount);
    a.paid += num(r.paid_amount);
    a.outstanding += num(r.outstanding_amount);
    if (num(r.overdue_days) > 0 && num(r.outstanding_amount) > 0) a.overdue += num(r.outstanding_amount);
    return a;
  }, { invoiced: 0, paid: 0, outstanding: 0, overdue: 0 }), [rows]);
  ...
  return <div className="space-y-4 pb-12 print-report" data-print-root>
    ...
    <section className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7" data-export-summary>
    ...
    <section className="no-print rounded-xl border border-slate-200 bg-white p-3" data-report-filters>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-7">
        <input className="input lg:col-span-2" placeholder="Search supplier, invoice, date, status or aging..." value={q} onChange={e => setQ(e.target.value)}/>
        <input type="date" className="input" value={from} onChange={e => setFrom(e.target.value)}/>
        <input type="date" className="input" value={to} onChange={e => setTo(e.target.value)}/>
        <select className="input" value={supplier} onChange={e => setSupplier(e.target.value)}><option value="">All Suppliers</option>{supplierNames.map(name => <option key={name} value={name}>{name}</option>)}</select>
        <select className="input" value={paymentStatus} onChange={e => setPaymentStatus(e.target.value)}><option value="">All Payment Statuses</option>{PAYMENT_STATUSES.map(s => <option key={s} value={s}>{s[0].toUpperCase() + s.slice(1)}</option>)}</select>
        <select className="input" value={agingBucket} onChange={e => setAgingBucket(e.target.value)}><option value="">All Aging Buckets</option>{AGING_BUCKETS.map(bucket => <option key={bucket} value={bucket}>{bucket}</option>)}</select>
      </div>
